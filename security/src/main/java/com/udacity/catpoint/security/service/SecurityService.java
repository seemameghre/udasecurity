...
public class SecurityService {
    ...
    public void setArmingStatus(ArmingStatus armingStatus) {
        Set<Sensor> activeSensors = new TreeSet<>();
...
//        catDetected(catIsDetected);
    ...
    private void catDetected(Boolean cat) {
        if(cat && getArmingStatus() == ArmingStatus.ARMED_HOME) {
            setAlarmStatus(AlarmStatus.ALARM);
        } else if(!securityRepository.isAnySensorActive()){
            setAlarmStatus(AlarmStatus.NO_ALARM);
        }
    ...
    private void handleSensorDeactivated() {
        if(securityRepository.getAlarmStatus() == AlarmStatus.ALARM){
            //Alarm is active. Sensors should have no effect.
            return;
        }
...
/*        if(!sensor.getActive() && active) {
        ...
        if(active){
            //sensor getting activated must always be handled
            sensor.setActive(active);
            securityRepository.updateSensor(sensor);
            handleSensorActivated();
        }else if(sensor.getActive()){
            //handle sensor deactivation only if it was active
            sensor.setActive(active);
            securityRepository.updateSensor(sensor);
            handleSensorDeactivated();
        }
